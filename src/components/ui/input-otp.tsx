...
export const InputOtp: React.FC<OtpInputProps> = ({ length = 6, onChange }) => {
    ...
    <div className="flex gap-2 sm:gap-4">
      {otp.map((digit, index) => (
        <input
          ...
          onPaste={handlePaste}
          className="focus:ring-primary-500 h-10 w-10 rounded-md border text-center text-xl focus:ring-1 focus:outline-none sm:h-12 sm:w-12"
        />
      ))}
    </div>
  );
