      ...
      <ActionTooltip label={name} side="right" align="center">
        <div
          ...
          <div className="group relative mx-3 flex size-10 overflow-hidden">
            {logo ? (
              <img
                className="size-9 sm:size-10 rounded-xl object-cover object-center"
                src={logo}
                alt={name}
              />
            ) : (
              <div
                className={cn(
                  "bg-secondary flex size-9 sm:size-10 items-center justify-center rounded-xl text-lg font-medium transition-all hover:bg-indigo-500 hover:text-white dark:hover:bg-indigo-500",
